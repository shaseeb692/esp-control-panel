...
export default function VerifyPage() {
  ...
  return (
    <main className={`min-h-screen px-4 py-8 ${darkMode ? "text-white" : "text-slate-950"}`}>
      <div className="mx-auto flex min-h-[90vh] max-w-md items-center justify-center">

        <div className={`w-full rounded-[30px] border p-7 shadow-2xl backdrop-blur-2xl ${glass}`}>

          <button
            onClick={() =>
              router.push("/auth/login")
            }
            className={`mb-8 flex items-center gap-2 text-sm ${muted}`}
          >
            <ArrowLeft size={16} />
            Back
          </button>
          ...
          <form onSubmit={verifyOtp}>
            ...
            <label className={`mb-2 block text-sm font-medium ${muted}`}>
              Verification code
            </label>
            ...
            <input
              ...
              placeholder="000000"
              className={`w-full rounded-2xl border px-4 py-4 text-center text-2xl font-semibold tracking-[0.5em] outline-none transition focus:border-[#42B8C5] ${glassSoft}`}
            />
